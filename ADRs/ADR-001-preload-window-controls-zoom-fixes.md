...
3. **Debounced Bounds Recalculation**: Added a 50ms debounce wrapper to `updateActiveViewBounds()` and explicit 100ms delays on `maximize` and `unmaximize` window handlers to allow GTK layouts to fully settle before calculating viewport dimensions.
