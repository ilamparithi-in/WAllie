  ...
  try {
    if (fs.existsSync(SETTINGS_FILE)) {
      ...
      return {
        ...
        ctrlScrollZoomEnabled: parsed.ctrlScrollZoomEnabled !== false,
        preloadAccountIds,
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        return parsed.map((acc: any) => ({
          ...
          settings: {
            ...
            fontUrl: acc.settings?.fontUrl || '',
            preferGoogleFont: acc.settings?.preferGoogleFont === true,
            monoFontFamily: acc.settings?.monoFontFamily || '',
            monoFontUrl: acc.settings?.monoFontUrl || '',
            preferGoogleMonoFont: acc.settings?.preferGoogleMonoFont === true,
